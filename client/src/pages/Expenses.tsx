...
export default function Expenses() {
  ...
  const handleExportExcel = () => {
    ...
    try {
      ...
      const currentDate = format(getAdjustedTime(), "MMM dd, yyyy hh:mm a");
      
      const headers = ["Date", "Category", "Description", "Amount (₹)"];
      const csvContent = [
        `"Expense Report - ${adminName}"`,
        ...
        `"Total Amount: ₹${sortedExpenses.reduce((sum, exp) => sum + parseFloat(exp.amount), 0).toFixed(2)}"`,
        "",
        `"Provided by Airavoto Gaming"`
      ].join("\n");
