...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Mini Webview Routes
  app.get("/api/mini-webview/live-data", publicApiLimiter, async (req, res) => {
    try {
      const [allBookings, deviceConfigs, pricingConfigs, facilities, centerInfo, games, settings] = await Promise.all([
        storage.getAllBookings(),
        storage.getAllDeviceConfigs(),
        storage.getAllPricingConfigs(),
        storage.getAllFacilities(),
        storage.getGamingCenterInfo(),
        storage.getAllGames(),
        storage.getMiniWebviewSettings()
      ]);
      ...
      const availability = deviceConfigs.map(config => {
        ...
        const available = config.count - activeBookings.length;
        const percentage = config.count > 0 ? Math.round((available / config.count) * 100) : 0;
        ...
        return {
          ...
          available,
          percentage,
          games: games.filter(game => game.category === config.category)
        };
      });
      
      const pricing = pricingConfigs.reduce((acc, config) => {
        ...
        return acc;
      }, {} as Record<string, Array<{ duration: string; price: string }>>);
      ...
      res.json({
        ...
        facilities,
        centerInfo,
        settings: settings || { isLiveEnabled: "true", refreshInterval: 5 }
      });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  ...
  app.get("/api/mini-webview/settings", requireAdmin, async (req, res) => {
    try {
      const settings = await storage.getMiniWebviewSettings();
      res.json(settings || { isLiveEnabled: "true", refreshInterval: 5 });
    } catch (error: any) {
      res.status(500).json({ message: error.message });
    }
  });

  app.post("/api/mini-webview/settings", requireAdmin, async (req, res) => {
    try {
      const settings = await storage.upsertMiniWebviewSettings(req.body);
      ...
      if (req.session.userId && req.session.username && req.session.role) {
        await storage.createActivityLog({
          ...
          userRole: req.session.role,
          action: 'update',
          entityType: 'mini-webview-settings',
          entityId: settings.id,
          details: `Updated mini webview settings`
        });
      }
